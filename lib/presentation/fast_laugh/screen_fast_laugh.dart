...
import 'package:mynetflix/presentation/fast_laugh/widgets/video_list.dart';
...
class ScreenFastLaugh extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    WidgetsBinding.instance!.addPostFrameCallback((_) {
      BlocProvider.of<FastLaughBloc>(context).add(FastLaughInitialEvent());
    });
    return Scaffold(
      body: SafeArea(
        child: BlocBuilder<FastLaughBloc, FastLaughState>(
          builder: (context, state) {
            if (state is FastlaughInitialSuccessState) {
              if (state.isLoading) {
                return const Center(
